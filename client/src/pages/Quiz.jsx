...
const Quiz = () => {
  ...
  if (gameState === 'lobby') {
    return (
      <div className="page-container animate-fade-in" style={{ padding: '1rem', maxWidth: '950px', margin: '0 auto' }}>
        
        {/* Header Title */}
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <BrainCircuit size={60} color="var(--accent-primary)" style={{ marginBottom: '0.75rem', filter: 'drop-shadow(0 0 15px rgba(136, 84, 192, 0.4))' }} />
          <h1 style={{ fontSize: '3rem', fontWeight: 900, fontFamily: 'var(--font-display)', letterSpacing: '-0.02em' }}>QUIZ CONFIGURE</h1>
        ...
        {/* Categories grid */}
        <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '1.25rem', fontFamily: 'var(--font-display)' }}>1. SELECT CATEGORY</h2>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  padding: '1.5rem',
                  gap: '0.5rem',
                  border: isActive ? `3px solid ${cat.color}` : '2px solid rgba(255,255,255,0.03)',
                  background: isActive ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.15)',
                  boxShadow: isActive ? `0 8px 24px -5px ${cat.color}33` : 'none',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <span style={{ fontWeight: 800, fontSize: '1.15rem', color: 'white' }}>{cat.name}</span>
                </div>
        ...
        {/* Difficulty grid */}
        <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '1.25rem', fontFamily: 'var(--font-display)' }}>2. CHOOSE DIFFICULTY</h2>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  padding: '1.5rem',
                  gap: '0.5rem',
                  border: isActive ? `3px solid ${diff.color}` : '2px solid rgba(255,255,255,0.03)',
                  background: isActive ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.15)',
                  boxShadow: isActive ? `0 8px 24px -5px ${diff.color}33` : 'none',
                  opacity: isActive ? 1 : 0.75,
                  borderRadius: '20px',
                  cursor: 'pointer'
                }}
  ...
  if (gameState === 'loading') {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh', color: 'var(--text-secondary)' }}>
        <div style={{ textAlign: 'center' }}>
          <BrainCircuit className="animate-float" size={60} color="var(--accent-secondary)" style={{ marginBottom: '1rem', filter: 'drop-shadow(0 0 15px rgba(0, 229, 255, 0.4))' }} />
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800 }}>Consulting Gemini LLM...</h2>
  ...
  if (gameState === 'finished') {
    return (
      <div style={{ maxWidth: '600px', margin: '3rem auto', padding: '1rem' }}>
        ...
        >
          <Trophy size={90} color="#fbbf24" style={{ marginBottom: '1.5rem', filter: 'drop-shadow(0 0 20px rgba(251, 191, 36, 0.4))' }} />
          <h2 style={{ fontSize: '2.75rem', fontWeight: 900, marginBottom: '0.5rem', fontFamily: 'var(--font-display)' }}>QUEST COMPLETED!</h2>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', fontWeight: 600 }}>Category: <span style={{ color: 'white' }}>{categoryName}</span></p>
          ...
          <div style={{ 
            ...
            <div style={{ fontSize: '4rem', fontWeight: 900, color: 'var(--accent-secondary)', fontFamily: 'var(--font-display)', margin: '0.25rem 0' }}>{score}</div>
          </div>

          {/* AI Game Master Performance Review */}
          {aiEvaluation && (
            <div className="card" style={{ 
              background: 'linear-gradient(135deg, rgba(136, 84, 192, 0.15), rgba(0, 229, 255, 0.05))', 
              borderColor: 'rgba(136, 84, 192, 0.25)', 
              padding: '1.5rem',
              marginBottom: '2.5rem',
              textAlign: 'left'
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--accent-secondary)', marginBottom: '0.5rem', fontWeight: 800, fontSize: '0.85rem' }}>
                <Sparkles size={16} /> 🔮 GEMINI AI SCORING EVALUATION
              </div>
              <p style={{ color: 'white', fontSize: '0.95rem', lineHeight: 1.6, fontWeight: 500 }}>
                "{aiEvaluation}"
              </p>
            </div>
          )}
  ...
  return (
    <div style={{ maxWidth: '850px', margin: '0 auto', padding: '1rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <span style={{ color: 'var(--accent-secondary)', fontWeight: 800, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            Gemini AI Category: {categoryName}
          </span>
          <h2 style={{ fontSize: '1.75rem', fontWeight: 800 }}>Question {currentIndex + 1} of {questions.length}</h2>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <div style={{ 
            ...
            alignItems: 'center', 
            gap: '0.5rem', 
            color: timeLeft <= 5 ? 'var(--error)' : 'var(--accent-secondary)',
          ...
          <div className="card" style={{ padding: '0.5rem 1.25rem', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', borderBottomWidth: '4px' }}>
            <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: 700 }}>Score: </span>
            <span style={{ fontWeight: 900, color: 'var(--warning)', fontSize: '1.1rem' }}>{score}</span>
          </div>
        </div>
      </div>
